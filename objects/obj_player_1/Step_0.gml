...
scr_get_input(player_id);
//depth = -y
script_execute(state);

if gamepad_button_check(player_id,gp_face1) or keyboard_check(ord("Z")) or keyboard_check(vk_numpad2) {
	i += 1;
	i = i mod 4;
	if (i == 0) {
		if ammo >= 1 {
...
if gamepad_button_check_released(player_id,gp_face1) or keyboard_check_released(ord("Z")) or keyboard_check_released(vk_numpad2) {
	ammo = pow;
	...
	if global.spellgauge[player_id] >= 256 {
		...
		if global.spellgauge[player_id] >= 256 and global.spellgauge[player_id] <= 383 {
			var inst = instance_create_depth(192,32,0,obj_charged);
			with (inst) {
...
if keyboard_check_pressed(ord("X")) or gamepad_button_check_pressed(player_id,gp_face2) {
	if global.bombs[player_id] >= 1 and !instance_exists(obj_bomb) {
		var inst = instance_create_depth(8,24,0,obj_bomb);
		inst.user = player_id;
